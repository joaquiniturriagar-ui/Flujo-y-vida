...
const CATS = [
  ...
  {n:"Auto Bencina",g:"Transporte"},{n:"Autoseguro",g:"Transporte"},{n:"Auto Cuota",g:"Transporte"},{n:"Auto Mantencion",g:"Transporte"},{n:"Movilizacion / Pasajes",g:"Transporte"},
  {n:"Recreacion / Ocio",g:"Entretención"},{n:"Gym",g:"Entretención"},
  {n:"Celular",g:"Personal"},{n:"Gastos Propios",g:"Personal"},{n:"Farmacia",g:"Personal"},{n:"Ropa",g:"Personal"},{n:"Retiro Cajero",g:"Personal"},{n:"Aplicaciones",g:"Personal"},
...
const INIT_ASSETS = [
  {id:"cbk",name:"CommBank AUS",balance:0,currency:"AUD"}
];
...
function QuickEntry({onClose,onSaveExp,onSavePay,onSaveInc,debts,assets}) {
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)"}}>
      <div style={{background:"#14152a",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto",borderRadius:"24px 24px 0 0",padding:"20px 18px 32px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:20}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
            <span style={{fontSize:32,fontWeight:800,color:mode==="ingreso"?X.g:X.ac,fontFamily:"'JetBrains Mono'"}}>{cur==="AUD"?"A$":"$"}</span>
        ...
        {mode==="pago" && (
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
            {debts.map(d=>(<button key={d.id} onClick={()=>setTid(d.id)} style={{background:tid===d.id?`${X.g}22`:X.card,border:`1px solid ${tid===d.id?X.g:X.bdr}`,borderRadius:12,padding:"12px",color:"#fff",textAlign:"left"}}><div style={{display:"flex",justifyContent:"space-between"}}><span>{d.name}</span><span style={{color:X.r,fontWeight:800}}>{fmt(d.usado)}</span></div></button>))}
          </div>
        )}

        {mode==="ingreso" && (
          <>
            <input type="text" placeholder="Origen del ingreso..." value={desc} onChange={e=>setDesc(e.target.value)} style={{width:"100%",background:X.card,border:`1px solid ${X.bdr}`,borderRadius:14,padding:"14px",color:X.tx,marginBottom:16,outline:"none"}}/>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
              <label style={{fontSize:10,color:X.txD,fontWeight:700,textTransform:"uppercase"}}>¿A qué cuenta entró?</label>
              {assets.map(a=>(<button key={a.id} onClick={()=>setAid(a.id)} style={{background:aid===a.id?`${X.g}22`:X.card,border:`1px solid ${aid===a.id?X.g:X.bdr}`,borderRadius:12,padding:"12px",color:"#fff",textAlign:"left"}}><div style={{display:"flex",justifyContent:"space-between"}}><span>{a.name}</span><span style={{color:X.g,fontWeight:800}}>{a.currency==="AUD" ? `A$ ${fmt(a.balance)}` : fmt(a.balance)}</span></div></button>))}
            </div>
          </>
        )}
...
export default function App() {
  ...
  return(
    <div style={{minHeight:"100vh", background:X.bg, color:X.tx, fontFamily:"sans-serif", paddingBottom:100}}>
      ...
      <div style={{padding:"0 18px", maxWidth:480, margin:"0 auto", display:"flex", flexDirection:"column", gap:16}}>
        ...
        {view === "home" && (
          <>
                ...
                <div key={a.id}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      <span style={{fontSize:20,fontWeight:800,color:X.g,fontFamily:"'JetBrains Mono'"}}>A$ {fmt(a.balance)}</span>
            ...
            <Cd>
              <h3 style={{fontSize:12, fontWeight:800, margin:"0 0 14px", color:X.txM, textTransform:"uppercase"}}>Por Grupo vs Presupuesto</h3>
              {Object.entries(GROUPS).map(([g,info])=>{
                const s=grp[g]||0, b=bud[g]||0;
                return(<div key={g} style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,fontSize:11}}>
                    <span>{info.i} {g}</span>
                    <span style={{fontFamily:"'JetBrains Mono'",fontWeight:700}}>{fmt(s)}<span style={{color:X.txD}}>/{fmtS(b)}</span></span>
                  </div>
                  <Br p={b>0?pct(s,b):0} color={info.c} h={5}/>
                </div>);
              })}
            </Cd>
        ...
        {view === "budget" && (
          <Cd>
            ...
            <button onClick={()=>saveData({exps,debts,pays,assets,bud})} style={{width:"100%", padding:12, background:X.g, border:"none", borderRadius:10, color:"#fff", fontWeight:700}}>GUARDAR CAMBIOS</button>
          </Cd>
        )}
      ...
      {show && <QuickEntry onClose={()=>setShow(false)} onSaveExp={addExp} onSavePay={addPay} onSaveInc={addInc} debts={debts} assets={assets}/>}
    </div>
  );
}
