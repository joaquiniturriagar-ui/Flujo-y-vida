...
const CATS = [
  ...
  {n:"Bencina",g:"Transporte"},{n:"Cuota Auto",g:"Transporte"},{n:"Seguro Auto",g:"Transporte"},{n:"Mantención Auto",g:"Transporte"},
...
const Cd = ({children,s,alert,onClick}) => (
  <div onClick={onClick} style={{background:X.card,border:`1px solid ${alert?X.r:X.bdr}`,borderRadius:14,padding:"16px 18px",position:"relative",overflow:"hidden",cursor:onClick?"pointer":"default",...s}}>{children}</div>
);
...
function QuickEntry({onClose,onSaveExp,onSavePay,debts}) {
  ...
  const save=()=>{
    ...
    if(mode==="pago"){
      ...
      onSaveExp({id:Date.now(), date:today(), amount:n, desc:desc||"Gasto", card, category:"Otros", group:"Otros"});
    }
    onClose();
  };
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      <div style={{background:"#14152a",width:"100%",maxWidth:480,borderRadius:"24px 24px 0 0",padding:"20px 18px 32px"}}>
        ...
        {mode==="gasto" ? (
          <input type="text" placeholder="Descripción" value={desc} onChange={e=>setDesc(e.target.value)} style={{width:"100%",background:X.card,border:`1px solid ${X.bdr}`,borderRadius:14,padding:"14px",color:X.tx,marginBottom:16}}/>
        ) : (
        ...
        <button onClick={save} style={{width:"100%",border:"none",borderRadius:16,padding:"16px",fontSize:16,fontWeight:800,background:mode==="pago"?X.g:X.ac,color:"#fff"}}>GUARDAR</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(()=>{
    return onDataChange((data)=>{
      if(data){
        setExps(data.exps || []);
        setPays(data.pays || []);
        setDebts(data.debts || INIT_DEBTS);
        setBud(data.bud || {});
        setSynced(true);
      }
    });
  ...
  const tB = useMemo(()=> Object.values(bud).reduce((a,v)=>a+v, 0) || 1, [bud]);
  ...
  const dPlan = useMemo(() => {
    ...
    const tl = []; let mo = 0;
    while(list.some(d=>d.bal>0) && mo < 36) {
      mo++; let ex = dEx;
  ...
  return(
    <div style={{minHeight:"100vh", background:X.bg, color:X.tx, fontFamily:"sans-serif", paddingBottom:100}}>
      <div style={{padding:"20px 18px", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
        ...
        <input type="month" value={cm} onChange={e=>setCm(e.target.value)} style={{background:X.card, border:`1px solid ${X.bdr}`, borderRadius:10, color:"#fff", padding:"5px 10px"}}/>
      </div>
